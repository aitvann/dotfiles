...
let
  ...
  librewolfConfigPath2 = librewolfConfigPath;

  profilesPath =
    if isDarwin then "${librewolfConfigPath2}/Profiles" else librewolfConfigPath2;

  nativeMessagingHostsPath = if isDarwin then
    "${librewolfConfigPath}/NativeMessagingHosts"
  else
    "${librewolfConfigPath}/native-messaging-hosts";

  nativeMessagingHostsJoined = pkgs.symlinkJoin {
    name = "ff_native-messaging-hosts";
    paths = [
      # Link a .keep file to keep the directory around
      (pkgs.writeTextDir "lib/librewolf/native-messaging-hosts/.keep" "")
      # Link package configured native messaging hosts (entire Librewolf actually)
      cfg.finalPackage
    ]
  ...
  mkUserJs = prefs: extraPrefs: bookmarks:
    let
      prefs' = lib.optionalAttrs ([ ] != bookmarks) {
        "browser.bookmarks.file" = toString (librewolfBookmarksFile bookmarks);
  ...
  librewolfBookmarksFile = bookmarks:
    let
      indent = level:
    ...
    in pkgs.writeText "librewolf-bookmarks.html" ''
  ...
  mkNoDuplicateAssertion = entities: entityKind:
    ...
    in {
      ...
      message = ''
        Must not have a Librewolf ${entityKind} with an existing ID but
      '' + concatStringsSep "\n" (mapAttrsToList mkMsg duplicates);
    });

  wrapPackage = package:
    let
      # The configuration expected by the Librewolf wrapper.
      ...
      # The configuration expected by the Librewolf wrapper builder.
    ...
    in if package == null then
      ...
      (pkgs.wrapLibrewolf.override { config = bcfg; }) package { };
  ...
  imports = [
    (mkRemovedOptionModule [ "programs" "librewolf" "extensions" ] ''
      ...
      Extensions are now managed per-profile. That is, change from
        ...
        programs.librewolf.extensions = [ foo bar ];
  ...
  options = {
    programs.librewolf = {
      ...
      package = mkOption {
        ...
        example = literalExpression ''
          pkgs.librewolf.override {
            # See nixpkgs' librewolf/wrapper.nix to check which options you can use
      ...
      finalPackage = mkOption {
        type = with types; nullOr package;
        readOnly = true;
        description = "Resulting Librewolf package.";
      };
        ...
        type = types.attrsOf (types.submodule ({ config, name, ... }: {
          options = {
            ...
            settings = mkOption {
              type = types.attrsOf (jsonFormat.type // {
                description =
                  "Librewolf preference (int, bool, string, and also attrs, list, float as a JSON string)";
              });
              default = { };
              example = literalExpression ''
                {
            ...
            search = {
              ...
              engines = mkOption {
                ...
                description = ''
                  ...
                  be treated as builtin to Librewolf.
            ...
            extensions = mkOption {
              ...
              '';
              description = ''
                List of Librewolf add-on packages to install for this profile.
                ...
                inside Librewolf after the first installation.

                To automatically enable extensions add
                `"extensions.autoDisableScopes" = 0;`
                to
                [{option}`programs.librewolf.profiles.<profile>.settings`](#opt-programs.firefox.profiles._name_.settings)
              '';
            };
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      in {
        ...
        message = "Must have exactly one default Librewolf profile but found "
    ...
    warnings = optional (cfg.enableGnomeExtensions or false) ''
      Using 'programs.librewolf.enableGnomeExtensions' has been deprecated and
      will be removed in the future. Please change to overriding the package
      configuration using 'programs.librewolf.package' instead. You can refer to
      its example for how to do this.
    '';

    programs.librewolf.finalPackage = wrapPackage cfg.package;

    home.packages = lib.optional (cfg.finalPackage != null) cfg.finalPackage;

    home.file = mkMerge ([{
      "${librewolfConfigPath2}/profiles.ini" =
        mkIf (cfg.profiles != { }) { text = profilesIni; };

      "${nativeMessagingHostsPath}" = {
        source =
          "${nativeMessagingHostsJoined}/lib/librewolf/native-messaging-hosts";
        recursive = true;
      };
        ...
        (profile.search.default != null || profile.search.privateDefault != null
          || profile.search.order != [ ] || profile.search.engines != { }) {
            ...
            source = let
                ...
                engines = let
                  ...
                  processCustomEngineInput = input:
                    ...
                    // optionalAttrs (input ? icon) {
                      ...
                      loadPath = ''
                        [home-manager]/programs.librewolf.profiles.${profile.name}.search.engines."${
              ...
              # maliciously. We're modifying the search outside of Librewolf, but
