...
M.on_attach = function(_)
    ...
    vim.keymap.set("n", '[d', nndiag.goto_prev(), { silent = true, desc = 'GOTO PREVIOUS diagnostics' })
    vim.keymap.set("n", ']d', nndiag.goto_next(), { silent = true, desc = 'GOTO NEXT diagnostics' })
    -- stylua: ignore end
